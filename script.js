...
// Stop line sits before the zebra crossing (drawIntersection draws the stripe
// band at offsets boxHalf+7..boxHalf+21 = 42..56 from center). A car waiting
// at pos=124 overhangs the band by ~14 units (62% of its length); the largest
// pos whose leading edge clears the far stripe edge is 101, so we stop there.
const STOP_LINE=101;
...
function freshIntersection(){return {phase:0,phaseTime:0,phaseDuration:20,signalState:'green',amberTimer:0,queues:{N:[],E:[],S:[],W:[]},vehicles:[],served:0,totalWait:0,maxQueue:0,lastDecision:'Waiting to start',preempting:false,inboundEmergency:false,pedestrian:{active:false,timer:0,side:null},history:[]}}
  ...
  for(const id of IDS){
    ...
    for(const v of iState.vehicles){
      ...
      if(v.committed && v.pos>=430){
        ...
        // Corner exits: the vehicle leaves along its OUTGOING axis (it has
        // already turned), not along the inbound lane it arrived on.
        if(v.turnType&&v.turnType!=='straight')v.exitAxis=outD;
        v.exiting=true;
      }
      ...
      // Waiting starts when a vehicle is halted at the head of the queue.
      // Tied to STOP_LINE (not a magic constant): when STOP_LINE moved from
      // 124 to 101, a fixed threshold of 105 sat above it and avgWait
      // collapsed to 0.
      if(v.pos>=STOP_LINE-4&&!green&&!v.committed){v.wait+=dt;v.tripWait+=dt}
    }
    ...
    if(emVeh){
      ...
      if(!iState.preempting){
        iState.preempting=true;iState.inboundEmergency=false;
      ...
      if(emVeh.pos>320){
        ...
        if(nb){
          ...
          if(!ns2.preempting){
            ns2.preempting=true;ns2.inboundEmergency=true;
    ...
    }else if(iState.preempting&&!iState.inboundEmergency){
    ...
    }else if(iState.preempting&&iState.inboundEmergency&&!iState.vehicles.some(v=>v.emergency&&!v.passed)){
      // The pre-cleared vehicle has now arrived at this intersection and its
      // own emVeh branch takes over; clear the inbound flag so the resume
      // branch can fire once it has actually cleared the junction.
      iState.inboundEmergency=false;
    }
...
function posAtOffset(off){return (off+140)*460/280-30}

// --- Vehicle pose: the single place movement space is turned into screen space.
// The turn curve is anchored to the intersection BOX (not an arbitrary pos), so
// a committing car's drawn position is continuous where it leaves its lane.
const LANE_OFFSET=12, BOX_HALF=35, OFFSET_MAX=140;
const TURN_ENTRY_POS=posAtOffset(-BOX_HALF);   // 142.5 — where the car enters the box
const TURN_EXIT_POS=posAtOffset(OFFSET_MAX);   // 430   — where it hands off / exits
function pointOn(dir,off,cx,cy){
  if(dir==='N')return [cx-LANE_OFFSET,cy-off];
  if(dir==='S')return [cx+LANE_OFFSET,cy+off];
  if(dir==='E')return [cx+off,cy+LANE_OFFSET];
  return [cx-off,cy-LANE_OFFSET];
}
function vehiclePose(v,cx,cy){
  const off=posToOffset(v.pos);
  // Curve applies ONLY while traversing the junction (entry..exit). Beyond the
  // exit the straight-axis branch with exitAxis takes over — both paths give
  // the same point at TURN_EXIT_POS (posToOffset(430)=OFFSET_MAX), so the
  // handoff is seamless. Without this bound a corner-exiting car stayed drawn
  // at the curve endpoint (p clamped to 1) for its whole exit fade instead of
  // driving off the map along its outgoing axis.
  if(v.committed&&v.turnType&&v.turnType!=='straight'&&v.outD&&v.pos>TURN_ENTRY_POS&&v.pos<=TURN_EXIT_POS){
    const p=Math.max(0,Math.min(1,(v.pos-TURN_ENTRY_POS)/(TURN_EXIT_POS-TURN_ENTRY_POS)));
    const start=pointOn(v.d,-BOX_HALF,cx,cy);
    const end=pointOn(v.outD,OFFSET_MAX,cx,cy);
    const c1=[cx+(start[0]-cx)*0.3,cy+(start[1]-cy)*0.3];
    const c2=[cx+(end[0]-cx)*0.3,cy+(end[1]-cy)*0.3];
    const q=1-p;
    const x=q*q*q*start[0]+3*q*q*p*c1[0]+3*q*p*p*c2[0]+p*p*p*end[0];
    const y=q*q*q*start[1]+3*q*q*p*c1[1]+3*q*p*p*c2[1]+p*p*p*end[1];
    const tx=3*q*q*(c1[0]-start[0])+6*q*p*(c2[0]-c1[0])+3*p*p*(end[0]-c2[0]);
    const ty=3*q*q*(c1[1]-start[1])+6*q*p*(c2[1]-c1[1])+3*p*p*(end[1]-c2[1]);
    return {x,y,ang:Math.atan2(ty,tx)+Math.PI/2};
  }
  const axis=v.exitAxis||v.d;
  const [x,y]=pointOn(axis,off,cx,cy);
  return {x,y,ang:axis==='N'?0:axis==='S'?Math.PI:axis==='E'?Math.PI/2:-Math.PI/2};
}
...
function drawIntersection(id,iState){
  ...
  for(const v of iState.vehicles){
    if(v.passed)continue;
    const {x,y,ang}=vehiclePose(v,cx,cy);
...
function render(){
  ...
  // Add pulse animation on mode change
  if(typeof window !== 'undefined' && window.lastMode!==state.mode){
    modeBadge.classList.add('animate-pulse');
    setTimeout(()=>modeBadge.classList.remove('animate-pulse'), 1000);
  }
  if(typeof window !== 'undefined') window.lastMode=state.mode;
  // Add pulse animation on mode change
  if(typeof window !== 'undefined' && window.lastMode!==state.mode){
  ...
  $('decisions').innerHTML=state.decisions.map(d=>{    const icon=d.type==='preempt'?'🚨':d.type==='resume'?'✅':d.type==='dispatch'?'🚑':d.type==='pedestrian'?'🚶':d.type==='ped-clear'?'🚦':'🔁';    let color='var(--color-muted)';    if(d.type==='signalChange'){color='var(--color-primary)'};    if(d.type==='emergency'){color='var(--color-error)'};    if(d.type==='pedestrian'){color='var(--color-success)'};    return `<div class="flex items-start gap-2 mb-1 py-1 px-2 rounded hover:bg-white/5 transition-colors">` +      `<span class="flex-shrink-0 text-"+(d.type==="emergency"?"error":d.type==="pedestrian"?"success":"primary")+">${icon}</span>` +      `<div class="flex-1">` +        `<div class="text-xs font-mono text-"+color+">${new Date().toLocaleTimeString()}</div>` +        `<div class="text-xs font-medium">${d.id}: ${d.reasoning}</div>` +      `</div>` +    `</div>`;  }).join('')||'<div class="text-xs opacity-50">No decisions yet — start the simulation.</div>';  $('runDot').className='inline-block w-2 h-2 rounded-full '+(state.running?'bg-success shadow-[0_0_8px_var(--color-success)]':'bg-base-300');
